...
n_outputs = 25
n_reservoir =500 # number of recurrent units300
leak_rate = 0.5 # leaking rate (=1/time_constant_of_neurons)
spectral_radius = 1.25 # Scaling of recurrent matrix
input_scaling = 1. # Scaling of input matrix
proba_non_zero_connec_W = 0.1 # Sparsity of recurrent matrix: Perceptage of non-zero connections in W matrix
proba_non_zero_connec_Win = 1. # Sparsity of input matrix
proba_non_zero_connec_Wfb = 1. # Sparsity of feedback matrix
regularization_coef =  1e-6 #None # regularization coefficient, if None, pseudo-inverse is use instead of ridge regression
...
# print( 'MSE = ' + str( mse ))
mse1 = np.mean((test_out[:,0] - output_pred[0][:,0])**2) # Mean Squared Error: see https://en.wikipedia.org/wiki/Mean_squared_error
mse2 = np.mean((test_out[:,1] - output_pred[0][:,1])**2)
#mse3 = np.mean((test_out[:,2] - output_pred[0][:,2])**2)
...
print("\nMean Squared error (MSE):\t\t%.4e" % (mse1) )
print("\nMean Squared error (MSE):\t\t%.4e" % (mse2) )
#print("\nMean Squared error (MSE):\t\t%.4e" % (mse3) )
...
for i in range(n_outputs):
    ...
    plt.plot(line[trainLen:trainLen+testLen],test_out[:, i], color='b', lw=1.5)
    plt.plot(line[trainLen:trainLen+testLen],output_pred[0][:, i], color='r', lw=1.5)
    plt.plot(train_out[:, i],color='g',lw=1.5)
    plt.savefig(str(i)+'.png')
    #plt.ylim([-40, 60])
    plt.title("Ouput predictions against real timeseries")
    plt.legend(["real timeseries", "output predictions","train_out"])
