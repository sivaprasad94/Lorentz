            ...
            for t in range(inp.shape[0]): # for each time step in the input
                ...
                if self.Wfb is None:
                    if t<inp.shape[0]-150:
                        ...
                        x = (1 - self.lr) * x + self.lr * np.tanh(np.dot(self.Win, np.hstack((1,all_outputs[j][:,t-1])).reshape(di, 1)) + np.dot(self.W, x))
                        #y = np.dot(self.Wout, np.vstack((1, x))).astype(self.typefloat)
                    #p = (1 - self.lr) * x + self.lr * np.tanh(np.dot(self.Win, u[t, :].reshape(di, 1)) + np.dot(self.W, x))
                else:
                ...
                #y = np.dot( self.Wout, np.vstack((1,p)) ).astype(self.typefloat)
