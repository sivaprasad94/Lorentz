...
trainLen = initLen + 4500 # number of time steps during which we train the network
testLen = 3000 # number of time steps during which we test/run the network
...
outputs=np.zeros([data.shape[0],n_outputs])
t=0
...
train_in = inputs[None,0:trainLen]
train_out = outputs[0:trainLen,:]
test_in = inputs[None,trainLen:trainLen+testLen]
test_out = outputs[trainLen:trainLen+testLen,:]
...
print("\nMean Squared error (MSE1):\t\t%.4e" % (mse1) )
#print("\nMean Squared error (MSE2):\t\t%.4e" % (mse2) )
...
for i in range(n_outputs):
    ...
    plt.plot(test_out[:,i], color='coral', lw=1.0)
    plt.plot(output_pred[0][:,i], color='0.00', lw=1.5)
