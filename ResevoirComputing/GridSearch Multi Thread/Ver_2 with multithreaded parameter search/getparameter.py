        ...
        for t in range(len(proba_non_zero_connec_W_op)):
            for r in range(len(regularization_coef_op)):
                ...
                nmrse_maxmin = rmse / abs(
                    np.max(test_out[:]) - np.min(test_out[:]))  # Normalised RMSE (based on max - min)
                #print("\n********************")
                #print("Errors computed over %d time steps" % (errorLen))
                print("n_reservoir="+str(n_reservoir)+" leak_rate="+str(leak_rate)+" proba_non_zero_connec_W="+str(proba_non_zero_connec_W)+" regularization_coef="+str(regularization_coef))
                # print("\nMean Squared error (MSE):\t\t%.4e" % (mse3) )
                print("\nMean Squared error (MSE):\t\t%.4e" % (mse))
                #print("Root Mean Squared error (RMSE):\t\t%.4e\n" % rmse)
                #("Normalized RMSE (based on mean):\t%.4e" % (nmrse_mean))
...
print("best mse="+str(minmse))
print("n_reservoir="+str(n_reservoir_bst)+" leak_rate="+str(leak_rate_bst)+" proba_non_zero_connec_W="+str(proba_non_zero_connec_W_bst)+" regularization_coef="+str(regularization_coef_bst))
